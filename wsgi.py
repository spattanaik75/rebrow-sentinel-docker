...
from flask import Flask, request, redirect, url_for, abort, render_template, flash, Markup, Response, json
import time
...
application.secret_key = os.getenv("SECRET_KEY", "lasfuoi3ro8w7gfow3bwiubdwoeg7p23r8g23rg")
...
def login():
    ...
    if request.method == 'POST':
        ...
        _host = url_encoder(request.form["host"])
        _port = url_encoder(request.form["port"])
        _db = url_encoder(request.form["db"])
        _password = url_encoder(request.form["password"])
        url = url_for("server_db", host=_host, port=_port, password=_password, db=_db)
        return redirect(url)
    else:
        s = time.time()
...
@application.route("/<host>:<port>:<password>/<db>/")
def server_db(host, port, password, db):
    """
    List all databases and show info on server
    """
    _host = url_decoder(host)
    _port = int(url_decoder(port))
    _password = url_decoder(password)
    _db = int(url_decoder(db))
    s = time.time()
    r = redis_client.conn(host=_host, port=_port, db=_db, password=_password)
    info = r.info("all")
    dbsize = r.dbsize()
...
@application.route("/<host>:<port>:<password>/<db>/keys/", methods=['GET', 'POST'])
def keys(host, port, password, db):
    """
    ...
    if request.method == "POST":
        ...
        application.logger.debug(action)
        if action == "delkey":
        ...
        limited_keys = [l.decode() for l in keys[offset:(perpage + offset)]]
        types = {}
...
@application.route("/<host>:<port>:<password>/<db>/keys/<key>/")
def key(host, port, password, db, key):
    """
    Show a specific key.
...
@application.route("/<host>:<port>:<password>/<db>/pubsub/")
def pubsub(host, port,password, db):
    """
...
def pubsub_event_stream(host, port, password, db, pattern):
    _host = url_encoder(request.form["host"])
    _port = url_encoder(request.form["port"])
    _db = url_encoder(request.form["db"])
    _password = url_encoder(request.form["password"])
...
@application.route("/<host>:<port>:<password>/<db>/pubsub/api/")
def pubsub_ajax(host, port, password, db):
    return Response(pubsub_event_stream(host, port, password, db, pattern="*"),
                    mimetype="text/event-stream")


@application.template_filter('url_encoder')
def url_encoder(s):
    if type(s)==bytes:
...
@application.template_filter('url_decoder')
def url_decoder(s):
    return base64.urlsafe_b64decode(s).decode('utf-8')


@application.template_filter('urlsafe_base64')
